...
- **[Collection.cs](Definitions/Collection.cs)**: Added `ValidateSystemCollectionStructure` method and integrated it into the `Insert` flow.
- **[DatabaseEngine.cs](DatabaseEngine.cs)**: Updated `DeleteCollection` to throw an error for system collections.
- **[app.js](wwwroot/js/app.js)**: Grouped collections starting with `sys` in a new section in the sidebar.
